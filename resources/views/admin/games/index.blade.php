        ...
        <table class="w-full">
            ...
            @foreach($games as $game)
                <tr x-data="{ showUpload: false }">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="relative group">
                            ...
                            <div class="w-16 h-16 rounded overflow-hidden">
                                @if($game->image)
                                    <img src="{{ asset($game->image) }}" alt="{{ $game->name }}" class="w-full h-full object-cover">
                                @else
                        ...
                        <div x-show="showUpload"
                            ...
                            <div class="bg-white p-6 rounded-lg shadow-xl max-w-md w-full" @click.stop>
                                <h3 class="text-lg font-bold mb-4">Update Image for {{ $game->name }}</h3>

                                <form action="{{ route('admin.games.update-image', $game) }}"
                                      method="POST"
                    ...
                    <td class="px-6 py-4">
                                <span class="px-2 py-1 text-sm {{ $game->current_rtp >= $game->theoretical_rtp ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} rounded-full">
